...
def main(
  ...
  with ProcessPoolExecutor(max_workers=workers, mp_context=get_context("spawn")) as executor:
    ...
    for future in tqdm(futures, desc="Running marker workers..."):
      times, min_time_worker, max_time_worker, max_vram, pages, chars = future.result()
      vrams.append(max_vram - current_gpu_vram)
      all_times.append(times)
      page_count += pages
      char_count += chars
      min_time = min(min_time_worker, min_time) if min_time is not None else min_time_worker
