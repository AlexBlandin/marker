...
def inference_tables(dataset, use_llm: bool, table_rec_batch_size: int | None, max_rows: int, use_gemini: bool):
  ...
  config_parser = ConfigParser(
    {"output_format": "json", "use_llm": use_llm, "table_rec_batch_size": table_rec_batch_size, "disable_tqdm": True}
  )
  total_unaligned = 0
  results = []
  ...
  for i in tqdm(range(iterations), desc="Converting Tables"):
    try:
      ...
      table_images = [
        page_image.crop(
          PolygonBox.from_bbox(bbox).rescale((page_bbox[2], page_bbox[3]), (page_image.width, page_image.height)).bbox
        )
      ...
      for table_idx, alignment in enumerate(table_alignments):
        ...
        aligned_tables.append((marker_tables[aligned_idx], gt_tables[table_idx], gemini_html))
      ...
      for marker_table, gt_table, gemini_table in aligned_tables:
        ...
        results.append(
          {"marker_table": marker_table_html, "gt_table": gt_table_html, "gemini_table": gemini_table_html}
        )
