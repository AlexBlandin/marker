...
@click.option(
  "--result_path",
  type=str,
  default=os.path.join(settings.OUTPUT_DIR, "benchmark", "table"),
...
def main(
  ...
  with ProcessPoolExecutor(max_workers=max_workers) as executor:
    marker_results = list(
      tqdm(executor.map(update_teds_score, results), desc="Computing alignment scores", total=len(results))
    )
  ...
  if use_gemini:
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
      gemini_results = list(
        tqdm(
          executor.map(update_teds_score, results, repeat("gemini")), desc="Computing Gemini scores", total=len(results)
        )
      )
  ...
  results = {"marker": marker_results, "gemini": gemini_results}
