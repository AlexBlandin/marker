...
def upload_and_process_file(api_key: str, fname: str, buff):
  ...
  files = {"file": (fname, buff, "application/pdf"), "purpose": (None, "ocr")}

  upload_response = requests.post("https://api.mistral.ai/v1/files", headers=upload_headers, files=files)
  ...
  url_response = requests.get(f"https://api.mistral.ai/v1/files/{file_id}/url?expiry=24", headers=url_headers)
  ...
  ocr_response = requests.post("https://api.mistral.ai/v1/ocr", headers=ocr_headers, json=ocr_data)
