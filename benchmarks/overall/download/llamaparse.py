...
def upload_and_parse_file(api_key: str, fname: str, buff, max_retries: int = 180, delay: int = 1):
  ...
  response = requests.post("https://api.cloud.llamaindex.ai/api/v1/parsing/upload", headers=headers, files=files)
  response.raise_for_status()
  job_id = response.json()["id"]

  # Poll for completion
  for _ in range(max_retries):
    status_response = requests.get(f"https://api.cloud.llamaindex.ai/api/v1/parsing/job/{job_id}", headers=headers)
    status_response.raise_for_status()
    if status_response.json()["status"] == "SUCCESS":
      # Get results
      result_response = requests.get(
        f"https://api.cloud.llamaindex.ai/api/v1/parsing/job/{job_id}/result/markdown", headers=headers
      )
