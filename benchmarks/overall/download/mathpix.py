...
def mathpix_request(buffer, headers):
  response = requests.post(
    ...
    headers=headers,
    data={"options_json": json.dumps({"conversion_formats": {"md": True, "html": True}})},
    files={"file": buffer},
  )
  data = response.json()
  pdf_id = data["pdf_id"]
...
def mathpix_status(pdf_id, headers):
  ...
  while i < max_iters:
    time.sleep(1)
    response = requests.get(f"https://api.mathpix.com/v3/converter/{pdf_id}", headers=headers)
...
def mathpix_results(pdf_id, headers, ext="md"):
  response = requests.get(f"https://api.mathpix.com/v3/converter/{pdf_id}.{ext}", headers=headers)
