...
class MarkdownCleaner:
  ...
  def normalize_markdown(md_text: str) -> str:
    with tempfile.TemporaryDirectory() as tmp_dir:
      ...
      subprocess.run(
        ["pandoc", str(input_file), "-f", "markdown+tex_math_dollars", "-t", "html", "-o", str(html_file), "--quiet"],
        check=True,
      )
      ...
      subprocess.run(
        ["pandoc", str(html_file), "-f", "html", "-t", "markdown+tex_math_dollars", "-o", str(output_file), "--quiet"],
        check=True,
      )
