...
class HeuristicScorer(BaseScorer):
  def __call__(self, sample, gt_markdown: List[str], method_markdown: str) -> BlockScores:
    if not method_markdown:
      return {"score": 0, "specific_scores": {"order": 0, "by_block": [0] * len(gt_markdown)}}
  ...
  @staticmethod
  def find_fuzzy_alignments(main_string: str, substrings: List[str], threshold: int = 70) -> List[dict]:
    alignments = []
    ...
    for idx, substr in enumerate(substrings):
      ...
      alignments.append({"string": substr, "start": dest_start, "end": dest_end, "score": score, "idx": idx})
    return alignments
