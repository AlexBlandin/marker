...
def get_method_scores(
  benchmark_dataset: datasets.Dataset, methods: List[str], score_types: List[str], artifacts: dict, max_rows=None
) -> FullResult:
  bench_scores = {}
...
@click.option(
  "--methods",
  type=str,
  help="Comma separated list of other methods to compare against.  Possible values: marker,mathpix,llamaparse,docling,mistral",
  default="marker",
)
@click.option(
  "--scores",
  type=str,
  help="Comma separated list of scoring functions to use.  Possible values: heuristic,llm",
  default="heuristic",
)
@click.option(
  "--result_path",
  type=str,
  default=os.path.join(settings.OUTPUT_DIR, "benchmark", "overall"),
