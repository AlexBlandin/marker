  ...
  def marker_api(self):
    ...
    async def convert_document(
      ...
      if output_format not in ["markdown", "json", "html", "chunks"]:
        raise HTTPException(status_code=400, detail="Output format must be one of: markdown, json, html, chunks")
...
async def invoke_conversion(pdf_file: Optional[str] = None, output_format: str = "markdown", env: str = "main"):
  """
  ...
  #
  try:
    service = modal.Cls.from_name("datalab-marker-modal-demo", "MarkerModalDemoService", environment_name=env)
