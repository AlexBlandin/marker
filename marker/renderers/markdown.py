  ...
  def convert_p(self, el, text, parent_tags):
    ...
    if has_continuation:
      ...
      if block_type in [BlockTypes.TextInlineMath, BlockTypes.Text]:
        if regex.compile(rf".*[\p{{Ll}}|\d][{hyphens}]\s?$", regex.DOTALL).match(
          text
        ...
        for r in range(rowspan):
          for c in range(colspan):
            try:
              ...
              logger.info(f"Overflow in columns: {col_idx + c} >= {total_cols} or rows: {row_idx + r} >= {total_rows}")
              continue
...
class MarkdownRenderer(HTMLRenderer):
  ...
  html_tables_in_markdown: Annotated[bool, "Return tables formatted as HTML, instead of in markdown"] = False
