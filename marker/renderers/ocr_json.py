...
class OCRJSONRenderer(BaseRenderer):
  ...
  def extract_json(self, document: Document) -> List[OCRJSONPageOutput]:
    ...
    for page in document.pages:
      page_equations = [b for b in page.children if b.block_type == BlockTypes.Equation and not b.removed]
      equation_lines = []
      for equation in page_equations:
        if not equation.structure:
          continue

        equation_lines += [line for line in equation.structure if line.block_type == BlockTypes.Line]

      page_lines = [
        block
        for block in page.children
        if block.block_type == BlockTypes.Line and block.id not in equation_lines and not block.removed
      ]
      ...
      for line in page_lines + page_equations:
        ...
        if line in page_equations:
          ...
          spans = [document.get_block(span_id) for span_id in line.structure] if line.structure else []
          children = []
          for span in spans:
            if not span.structure:
              continue

            span_chars = [document.get_block(char_id) for char_id in span.structure]
            children.extend(
              [
                OCRJSONCharOutput(
                  id=str(char.id),
