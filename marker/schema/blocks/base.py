...
class BlockMetadata(BaseModel):
  ...
  def merge(self, model2):
    return self.__class__(**{field: getattr(self, field) + getattr(model2, field) for field in self.model_fields})
...
class Block(BaseModel):
  ...
  def render(
    ...
    if self.structure is not None and len(self.structure) > 0:
      for block_id in self.structure:
        block = document.get_block(block_id)
        rendered = block.render(document, self.structure, section_hierarchy, block_config)
    ...
    return BlockOutput(
      html=self.assemble_html(document, child_content, parent_structure, block_config),
  ...
  def handle_html_output(self, document, child_blocks, parent_structure, block_config=None):
    ...
    html = Block.assemble_html(self, document, child_ref_blocks, parent_structure, block_config)
