...
class BaseTable(Block):
  ...
  @staticmethod
  def format_cells(document, child_blocks, block_config, child_cells: List[TableCell] | None = None):
    if child_cells is None:
      child_cells: List[TableCell] = [
        document.get_block(c.id) for c in child_blocks if c.id.block_type == BlockTypes.TableCell
      ]
    ...
    for row_id in unique_rows:
      ...
      for cell in row_cells:
        html_repr += cell.assemble_html(document, child_blocks, None, block_config)
      html_repr += "</tr>"
  ...
  def assemble_html(
    ...
    template = super().assemble_html(document, child_ref_blocks, parent_structure, block_config)
