...
from marker.cache import CACHE_DIR_NAME, save_document_cache
...
def process_single_pdf(args):
  ...
  try:
    ...
    if cli_options.get("save_document_state") and hasattr(converter, "document"):
      cache_dir = os.path.join(out_folder, CACHE_DIR_NAME)
      save_document_cache(converter.document, cache_dir)
...
def convert_cli(in_folder: str, **kwargs):
  ...
  with GPUManager(chunk_idx) as gpu_manager:
    ...
    kwargs["total_torch_threads"] = max(2, psutil.cpu_count(logical=False) // total_processes)
