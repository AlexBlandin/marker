...
def extract_data(fname: str, config: dict, schema: str, markdown: str | None = None) -> (str, Dict[str, Any], dict):
...
with col2:
  ...
  with tab1:
    ...
    default_json_value = st.session_state.rendered_pydantic_schema if st.session_state.rendered_pydantic_schema else ""
...
class Schema(BaseModel):
    ...
    if render_schema and pydantic_schema_input:
      try:
        ...
        schema = None
    elif pydantic_schema_input and pydantic_schema_input.strip() and not render_schema:
      # If there's Pydantic code but not rendered yet, show a message
      if "class Schema(BaseModel):" in pydantic_schema_input and "pass" not in pydantic_schema_input:
        st.info("💡 Click 'Render Pydantic schema to JSON' to convert your Pydantic model to JSON schema")

# Move the run logic outside of col2
run_marker = st.sidebar.button("Run Extraction")

use_llm = st.sidebar.checkbox("Use LLM", help="Use LLM for higher quality text", value=False)
...
if run_marker:
  if not schema:
    st.error("❌ Please provide a schema in either the JSON Schema or Pydantic Schema tab before running extraction.")
    st.stop()
  ...
  with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    try:
      rendered = extract_data(temp_pdf, cli_options, schema, st.session_state.markdown)
