...
class OcrBuilder(BaseBuilder):
  ...
  block_mode_max_height_frac: Annotated[
    float, "Max height of a block as a percentage of the page before falling back to line mode"
  ] = 0.5
  ...
  def __call__(self, document: Document, provider: PdfProvider):
    ...
    ocr_page_images, block_polygons, block_ids, block_original_texts = self.get_ocr_images_polygons_ids(
      document, pages_to_ocr, provider
    )
    self.ocr_extraction(
      document,
      pages_to_ocr,
  ...
  def get_ocr_images_polygons_ids(self, document: Document, pages: List[PageGroup], provider: PdfProvider):
      ...
      for block in document_page.structure_blocks(document):
        ...
        for block in blocks_to_ocr:
          ...
          block_polygon_rescaled = (
            copy.deepcopy(block.polygon).rescale(page_size, image_size).fit_to_bounds((0, 0, *image_size))
          )
          block_bbox_rescaled = block_polygon_rescaled.polygon
          block_bbox_rescaled = [[int(x) for x in point] for point in block_bbox_rescaled]
  ...
  def ocr_extraction(
    ...
    for document_page, page_recognition_result, page_block_ids, image in zip(
      ...
      for block_id, block_ocr_result in zip(page_block_ids, page_recognition_result.text_lines):
        ...
        all_line_spans = self.spans_from_html_chars(block_ocr_result.chars, document_page, image)
  ...
  def spans_from_html_chars(self, chars: List[TextChar], page: PageGroup, image: Image.Image) -> List[List[Span]]:
    ...
    for idx, char in enumerate(chars):
      char_box = PolygonBox(polygon=char.polygon).rescale(image_size, page.polygon.size)
