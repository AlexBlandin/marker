...
class LayoutBuilder(BaseBuilder):
  ...
  max_expand_frac: Annotated[float, "The maximum fraction to expand the layout box bounds by"] = 0.05
    ...
    for page in document.pages:
      ...
      for block_id in page.structure:
        ...
        if block.block_type in self.expand_block_types:
          ...
          if not other_blocks:
            block.polygon = block.polygon.expand(self.max_expand_frac, self.max_expand_frac).fit_to_bounds(
              (0, 0, *page_size)
            )
            continue
  ...
  def add_blocks_to_pages(self, pages: List[PageGroup], layout_results: List[LayoutResult]):
    for page, layout_result in zip(pages, layout_results):
      ...
      for bbox in sorted(layout_result.bboxes, key=lambda x: x.position):
        ...
        layout_block.polygon = layout_block.polygon.rescale(layout_page_size, provider_page_size).fit_to_bounds(
          (0, 0, *provider_page_size)
        )
        layout_block.top_k = {
          BlockTypes[label]: prob for (label, prob) in bbox.top_k.items() if label in BlockTypes.__members__
        }
