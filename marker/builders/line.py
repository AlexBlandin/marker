...
class LineBuilder(BaseBuilder):
  ...
  def get_detection_results(self, page_images: List[Image.Image], run_detection: List[bool]):
    ...
    page_detection_results = self.detection_model(images=page_images, batch_size=self.get_detection_batch_size())
  ...
  def get_all_lines(self, document: Document, provider: PdfProvider):
    ...
    for document_page, detection_result, provider_lines_good in zip(document.pages, detection_results, layout_good):
      provider_lines: List[ProviderOutput] = provider.page_lines.get(document_page.page_id, [])
  ...
  def ocr_error_detection(self, pages: List[PageGroup], provider_page_lines: ProviderPageLines):
    ...
    ocr_error_detection_results = self.ocr_error_model(page_texts, batch_size=int(self.get_ocr_error_batch_size()))
    return ocr_error_detection_results

  def check_line_overlaps(self, document_page: PageGroup, provider_lines: List[ProviderOutput]) -> bool:
    ...
    for i, line in enumerate(provider_lines):
      intersect_counts = np.sum(intersection_matrix[i] > self.provider_line_provider_line_min_overlap_pct)
  ...
  def merge_blocks(
    ...
    for document_page in document.pages:
      provider_lines: List[ProviderOutput] = page_provider_lines[document_page.page_id]
