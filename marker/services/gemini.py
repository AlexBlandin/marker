...
class BaseGeminiService(BaseService):
  gemini_model_name: Annotated[str, "The name of the Google model to use for the service."] = "gemini-2.0-flash"
  thinking_budget: Annotated[int, "The thinking token budget to use for the service."] = None
  ...
  def process_images(self, images):
    image_parts = [types.Part.from_bytes(data=self.img_to_bytes(img), mime_type="image/webp") for img in images]
    return image_parts
    ...
    for tries in range(1, total_tries + 1):
      ...
      try:
        responses = client.models.generate_content(
          ...
          + [prompt],  # According to gemini docs, it performs better if the image is the first element
          config=config,
        )
