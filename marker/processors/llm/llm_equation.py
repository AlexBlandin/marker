...
class LLMEquationProcessor(BaseLLMSimpleBlockProcessor):
  ...
  equation_latex_prompt: Annotated[
    str, "The prompt to use for generating LaTeX from equations.", "Default is a string containing the Gemini prompt."
...
"""
  ...
  def inference_blocks(self, document: Document) -> List[BlockData]:
    ...
    for block_data in blocks:
      ...
      if all([block.polygon.height / page.polygon.height < self.min_equation_height, not self.redo_inline_math]):
        continue
  ...
  def block_prompts(self, document: Document) -> List[PromptData]:
    ...
    for block_data in self.inference_blocks(document):
      ...
      prompt_data.append(
        {"prompt": prompt, "image": image, "block": block, "schema": EquationSchema, "page": block_data["page"]}
      )
