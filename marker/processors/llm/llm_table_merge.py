...
class LLMTableMergeProcessor(BaseLLMComplexBlockProcessor):
  ...
  table_start_threshold: Annotated[
    float, "The maximum percentage down the page the second table can start to be considered for merging."
  ] = 0.2
  vertical_table_height_threshold: Annotated[
    float, "The height tolerance for 2 adjacent tables to be merged into one."
  ] = 0.25
  vertical_table_distance_threshold: Annotated[int, "The maximum distance between table edges for adjacency."] = 20
  horizontal_table_width_threshold: Annotated[
    float, "The width tolerance for 2 adjacent tables to be merged into one."
  ] = 0.25
  horizontal_table_distance_threshold: Annotated[int, "The maximum distance between table edges for adjacency."] = 10
  ...
  table_merge_prompt: Annotated[
    str, "The prompt to use for rewriting text.", "Default is a string containing the Gemini rewriting prompt."
...
"""
  ...
  def rewrite_blocks(self, document: Document):
        ...
        if prev_block is not None:
          ...
          subsequent_page_table = all(
            [
              ...
              max(prev_block.polygon.height / page.polygon.height, block.polygon.height / page.polygon.height)
          ...
          same_page_new_column = all(
            [
              ...
              < block.polygon.width * (1 + self.vertical_table_height_threshold),  # Similar width
              col_match,
            ]
          )
          merge_condition = any(
            [subsequent_page_table, same_page_vertical_table, same_page_new_column, same_page_horizontal_table]
          )
    ...
    with ThreadPoolExecutor(max_workers=self.max_concurrency) as executor:
      for future in as_completed([executor.submit(self.process_rewriting, document, blocks) for blocks in table_runs]):
  ...
  def validate_merge(
    self, cells1: List[TableCell], cells2: List[TableCell], direction: Literal["right", "bottom"] = "right"
  ):
    if direction == "right":
