...
class LLMHandwritingProcessor(BaseLLMSimpleBlockProcessor):
  ...
  handwriting_generation_prompt: Annotated[
    str, "The prompt to use for OCRing handwriting.", "Default is a string containing the Gemini prompt."
...
"""
  ...
  def block_prompts(self, document: Document) -> List[PromptData]:
    ...
    for block_data in self.inference_blocks(document):
      ...
      prompt_data.append(
        {"prompt": prompt, "image": image, "block": block, "schema": HandwritingSchema, "page": block_data["page"]}
      )
    return prompt_data
