...
"""
  ...
  def process_rewriting(self, document: Document, page: PageGroup, block: Table):
    ...
    for i in range(0, row_count, self.max_rows_per_batch):
      ...
      batch_cell_bboxes = [
        cell.polygon.rescale(page.polygon.size, page.get_image(highres=True).size).bbox for cell in batch_cells
      ]
  ...
  def rewrite_single_chunk(
    ...
    if total_iterations < self.max_table_iterations and score < 4:
      logger.info(f"Table rewriting low score {score}, on iteration {total_iterations}")
    ...
    if not corrected_html.endswith("</table>"):
      logger.debug("Table parsing issue, corrected html does not end with </table>")
      block.update_metadata(llm_error_count=1)
      return
