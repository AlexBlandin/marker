  ...
  def __init__(self, config=None):
    ...
    for i, block_config_str in enumerate(self.block_relabel_str.split(",")):
      ...
      try:
        ...
        self.block_relabel_map[block_type] = (confidence_thresh, relabel_block_type)
      except Exception as e:
        logger.warning(
          f"Failed to parse relabel rule '{block_config_str}' at index {i}: {e}. Expected format is <original_label>:<new_label>:<confidence_threshold>"
        )
    ...
    for page in document.pages:
      for block in page.structure_blocks(document):
        ...
        if confidence > confidence_thresh:
          logger.debug(
            f"Skipping relabel for {block_id}; Confidence: {confidence} > Confidence Threshold {confidence_thresh} for re-labelling"
          )
          continue
