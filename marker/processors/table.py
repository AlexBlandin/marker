...
class TableProcessor(BaseProcessor):
  ...
  def align_table_cells(self, table: TableResult, table_detection_result: TextDetectionResult):
    ...
    for cell_idx, cell in enumerate(table_cells):
      ...
      intersecting_line_indices = [i for i, area in enumerate(intersection_matrix[:, cell_idx]) if area > 0]
      ...
      if non_assigned_lines:
        ...
        top_box = min(non_assigned_lines, key=lambda line: line.bbox[1])  # smallest y0
        bottom_box = max(non_assigned_lines, key=lambda line: line.bbox[3])  # largest y1
  ...
  def needs_ocr(self, tables: List[TableResult], table_blocks: List[dict]):
    ...
    for j, (table_result, table_block) in enumerate(zip(tables, table_blocks)):
      ...
      if table_block["ocr_block"] and text_lines_need_ocr and not self.disable_ocr:
        logger.debug(
  ...
  def assign_ocr_lines(self, tables: List[TableResult], table_blocks: list):
    ...
    det_images = [t["table_image"] for i, t in enumerate(table_blocks) if i in ocr_idxs]
    ...
    for result, ocr_res in zip(ocr_tables, ocr_results):
      ...
      assert len(cells_need_text) == len(ocr_res.text_lines), "Number of cells needing text and OCR results must match"
