...
class ExtractionConverter(PdfConverter):
  pattern: str = r"{\d+\}-{48}\n\n"
  existing_markdown: Annotated[str, "Markdown that was already converted for extraction."] = None
  ...
  def __call__(self, filepath: str) -> ExtractionOutput:
    ...
    self.config["output_format"] = "markdown"  # Output must be markdown for extraction
    ...
    if self.artifact_dict.get("llm_service") is None:
      self.artifact_dict["llm_service"] = self.resolve_dependencies(self.default_llm_service)
